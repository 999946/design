...
export default class Select extends React.Component <typings.PropsDefine, typings.StateDefine> {
    ...
    // 选择栏目点击
    handleClick(value: string, label: string, children?: Array<typings.Options>, zIndex: number = 1) {
     ...
     */
    getOptionItemByType(item: typings.Options, key: number, activeValue: string, zIndex: number = 1): React.ReactElement<any> {
        if (item.groupValue) {
            // 是一个分组
    ...
    render() {
        ...
        return (
            <Input {...others(new typings.Props(), this.props, ['children']) }
                {...extProps}
